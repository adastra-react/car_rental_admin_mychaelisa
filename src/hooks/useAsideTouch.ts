...
const useAsideTouch = () => {
	...
	useEffect(() => {
		if (!asideStatus) {
			x.set(0);
			// touchStatus must be forced closed here even though the x-subscriber
			// above will also fire from x.set(0); it runs first and would leave
			// touchStatus stale-open otherwise.
