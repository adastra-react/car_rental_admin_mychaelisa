...
export function getUserDataWithUsername(username: string): IUserProps | undefined {
	const key = Object.keys(USERS).find((f) => USERS[f].username.toString() === username);
	return key ? USERS[key] : undefined;
}

export function getUserDataWithId(id?: string): IUserProps | undefined {
	const key = Object.keys(USERS).find((f) => USERS[f].id.toString() === id?.toString());
	return key ? USERS[key] : undefined;
}
