...
const CommonDashboardIncome: FC<ICommonDashboardIncomeProps> = ({ activeTab }) => {
	...
	const salesChartOptions: IChartOptions['options'] = {
	...
	function getSalesForTab(tab: TTabs): {
		series: IChartOptions['series'];
		sales: { compare: number };
		campaigns: { price: number; compare: number };
		coupons: { price: number; compare: number };
	} {
		if (tab === TABS.MONTHLY) {
			return {
				series: [
	...
	const sales: IChartOptions = { ...getSalesForTab(activeTab), options: salesChartOptions };
	const salesSeriesData = (sales.series?.[0] as { data: number[] } | undefined)?.data ?? [];
								...
								<div className='d-flex align-items-center pb-3'>
									...
									<div className='flex-grow-1 ms-3'>
										<div className='fw-bold fs-3 mb-0'>
											{priceFormat(average(salesSeriesData))}
											<PercentComparison
												valueOne={average(salesSeriesData)}
