		...
		options: {
			...
			tooltip: {
				custom(opts) {
					...
					const ylabel = (w.config.series as any)[seriesIndex].data[dataPointIndex].x;

					return (
						`${
							'<div class="apexcharts-tooltip-rangebar">' +
							'<div> <span class="series-name" style="color: '
						}${color}">${seriesName || ''}</span></div>` +
						`<div> <span class="category">${ylabel} </span> <span class="value start-value">${fromYear}</span> <span class="separator">-</span> <span class="value end-value">${toYear}</span></div>` +
						`</div>`
					);
				},
			},
