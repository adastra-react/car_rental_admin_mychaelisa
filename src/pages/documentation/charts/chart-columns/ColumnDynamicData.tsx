...
import ApexCharts, { ApexFormatterOpts } from 'apexcharts';
import Card, {
	CardBody,
	CardHeader,
	CardLabel,
	...
	const [dynamicData] = useState<IChartOptions>({
		...
		options: {
			...
			chart: {
				...
				events: {
					dataPointSelection(e, chart, opts) {
						...
						if (opts!.selectedDataPoints[0].length === 1) {
							if (quarterChartEl?.classList.contains('active')) {
								updateQuarterChart(opts!, 'barQuarter');
							} else {
					...
					updated(chart, options) {
						updateQuarterChart(options!, 'barQuarter');
					},
				},
			},
			plotOptions: {
			...
			tooltip: {
				...
				y: {
					title: {
						formatter(seriesName: string, opts?: ApexFormatterOpts) {
