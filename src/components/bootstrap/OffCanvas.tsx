...
const OffCanvas: FC<IOffCanvasProps> = ({
	...
	useLayoutEffect(() => {
		setRightPanel(
			!!isRightPanel && !!deviceScreen?.width && deviceScreen.width > 1200 && isOpen,
		);
	});
	...
	// framer-motion's `motion` proxy can't be indexed by a union of tag names and
	// still infer the right component's prop type, so widen it deliberately here.
	const MotionTagWrapper = motion[Tag] as React.ComponentType<any>;
