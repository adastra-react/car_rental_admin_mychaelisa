...
const Carousel = forwardRef<HTMLDivElement, ICarouselProps>(
	(
		...
		const ITEMS = (items || children) as
			| { src?: string; altText?: string; captionHeader?: string; captionText?: string }[]
			| undefined;
		...
		const next = () => {
			if (animating) return;
			const nextIndex = activeIndex === (ITEMS?.length ?? 0) - 1 ? 0 : (activeIndex ?? 0) + 1;
			setActiveIndex(nextIndex);
		};

		const previous = () => {
			if (animating) return;
			const nextIndex = activeIndex === 0 ? (ITEMS?.length ?? 0) - 1 : (activeIndex ?? 0) - 1;
			setActiveIndex(nextIndex);
		};
		...
		const getSlideContent = (_item: {
			...
			// In children mode (no `items` prop), `_item` is actually the ReactNode child itself.
			return _item as ReactNode;
		};
