...
const Icon = forwardRef<HTMLSpanElement, IIconProps>(
	({ icon, className, color, size, forceFamily, ...props }, ref) => {
		...
		const SvgIconWrapper = (SvgIcon as Record<string, React.ComponentType<any>>)[IconName];
		// eslint-disable-next-line import/namespace
		const MaterialWrapper = (Material as Record<string, React.ComponentType<any>>)[IconName];
