...
const Item: FC<IItemProps> = ({
	...
	const WITHOUT_CHILD =
		...
		((typeof to === 'string' && ANCHOR_LINK_PATTERN.test(to) && (
			...
			<NavLink
				...
				className={({ isActive }) => classNames(LINK_CLASS, isActive ? 'active' : '')}
				to={`../${to}`}
	...
	const setButtonRef = useCallback((node: HTMLSpanElement | null, ref: RefHandler) => {
		dropdownButtonRef.current = node;
		return ref(node);
	}, []);

	const dropdownListRef = useRef<HTMLUListElement | null>(null);
	const setListRef = useCallback((node: HTMLUListElement | null, ref: RefHandler) => {
				...
				<Manager>
					<li
						...
						<Reference>
							{({ ref }) => (
								<span
									ref={(node) => setButtonRef(node, ref as RefHandler)}
						...
						{dropdownStatus && (
							<Popper
								...
								{({ ref, style, placement }) => (
									<List
										ref={(node) => setListRef(node, ref as RefHandler)}
										style={style}
